...
- Do not prefix commands with `mkosi box --`. Assume you are running within mkosi box,
so all commands execute within the right environment by default.

## Build and Test Commands

- Never compile individual files or targets. Always run `meson compile -C build` to build the entire project.
Meson handles incremental compilation automatically.
